      ...
      "games": [
        {
          ...
          "levels": [
            ...
            },
            {
              ...
              "from": 15,
              "to": 16,
              "title": "SSL/TLSで暗号化してパスを送る (openssl s_client)",
              "goal": "現レベルのパスを localhost:30001 へ SSL/TLS 経由で送ると次のパスが返る",
              "status": "done",
              "date": "2026-06-18",
              "boss": false,
              "tags": [
              ...
              "steps": [
                {
                  "cmd": "ssh bandit15@bandit.labs.overthewire.org -p 2220",
                  "note": "bandit15 でログイン"
                },
                {
                  "cmd": "openssl s_client -connect localhost:30001",
                  "note": "TLSクライアントで30001に接続。最初に証明書情報がドバっと出る=TLSハンドシェイク(暗号トンネル敷設)。終わると入力待ち"
                },
                {
                  "cmd": "openssl s_client -connect localhost:30001 -quiet",
                  "note": "-quiet で証明書の山を抑制＆入力先頭R/Qの特殊解釈を無効化。暗号化されたncのように使える(初心者はこっち推奨)"
                },
                {
                  "cmd": "8xCjnmgoKbGLhHFAZlGE5Tmu4M2tKJQo  (入力待ちに貼って Enter)",
                  "note": "暗号トンネルの中で現レベルのパスを送信→Correct!と共に bandit16 のパスが返る"
                }
              ],
              "takeaways": [
                "通信は『待ち受けるサーバー(service/daemon)』と『話しかけるクライアント(nc/openssl)』の2者。localhost:PORT=このマシンのPORT番窓口で待つプログラムに話しかける",
                "nc=平文TCP / openssl s_client=まずTLSハンドシェイクで暗号トンネルを作ってから喋る。http vs https と同じ仕組みを手動でやっている",
                "openssl s_client の罠: 入力行の先頭R=再ネゴシエーション/Q=切断と解釈される。-quiet で無効化できる"
              ]
            },
            {
              "id": "16-17",
              "from": 16,
              "to": 17,
              "title": "ポートスキャンで当たりを探しSSL接続→秘密鍵を奪取 (nmap+openssl)",
              "goal": "31000-32000をスキャン→SSLで喋る正解ポートにパスを送るとSSH秘密鍵が返る→鍵でbandit17へ",
              "status": "done",
              "date": "2026-06-18",
              "boss": true,
              "tags": [
              ...
              "steps": [
                {
                  "cmd": "nmap -p 31000-32000 localhost",
                  "note": "1000個のポートから open(待ち受け中)を炙り出す=recon。手でncは無理→スキャナの出番"
                },
                {
                  "cmd": "nmap -sV -p <openだったポート> localhost",
                  "note": "-sV でサービス種別を判定。'ssl' と出るのが正解ポート(今回は31790)。平文ダミーは送った文字をオウム返しするだけ"
                },
                {
                  "cmd": "openssl s_client -connect localhost:31790 -quiet",
                  "note": "正解ポートにSSLで接続しbandit16パスを送信→-----BEGIN RSA PRIVATE KEY----- で始まる秘密鍵が返る"
                },
                {
                  "cmd": "鍵を自PCに保存→icacls/chmod 600で自分だけ読める権限に→ssh -i key bandit17@bandit.labs.overthewire.org -p 2220",
                  "note": "サーバー内localhost:2220はブロック(13->14と同じ罠)→鍵を持ち出し外から鍵認証ログイン=lateral movement"
                }
              ],
              "takeaways": [
